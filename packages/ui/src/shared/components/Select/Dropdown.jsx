...
const SelectDropdown = ({
    ...
    useLayoutEffect(() => {
        const setFirstOptionAsActive = () => {
            ...
            if ($optionsRef.current.firstElementChild) {
                $optionsRef.current.firstElementChild.classList.add(
                    activeOptionClass
                );
            }
        };
    ...
    const handleInputArrowUpOrDownKeyDown = (event) => {
        ...
        if (event.keyCode === KeyCodes.ARROW_DOWN) {
            if ($options.lastElementChild === $active) {
                ...
                if (
                    $active.offsetTop >
                    $options.scrollTop + $optionsHeight / 1.4
                ) {
            ...
            if ($options.firstElementChild === $active) {
                ...
                if (
                    $active.offsetTop <
                    $options.scrollTop + $optionsHeight / 2.4
                ) {
                    $options.scrollTop -= $activeHeight;
                }
            }
    ...
    const isOptionCreatable =
        onCreate && searchValue && !isSearchValueInOptions;
...
const activeOptionClass = 'tmsimple-select-option-is-active';
